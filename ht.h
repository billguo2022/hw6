...
public:
    ...
    HASH_INDEX_T next() 
    {
        ...
        HASH_INDEX_T a = (this->start_ + this->numProbes_ * this->dhstep_) % this->m_;
        this->numProbes_++;
        return a;
...
void HashTable<K,V,Prober,Hash,KEqual>::insert(const ItemType& p)
{
    ...
    double load = double(in)/table_.size(); // calculate the load factor
    if (load >= max) { // if the load factor is above the maximum allowed
        resize(); // resize the table
        insert(p); // recursively call insert() with the same argument
    }
    // Find a position for the new item
    HASH_INDEX_T z= this->probe(p.first); // probe the table to find an empty or deleted position
    if (z == npos) { // if no position was found
        throw std::logic_error("no position");
    } else if (table_[z] != nullptr && !table_[z]->deleted) {
        // If an item with the same key already exists, update its value
        table_[z]->item = p;
    } else {
        double n = double(si)/table_.size(); // calculate the load factor
        if (n >= max) { // if the load factor is above the maximum allowed
            resize(); // resize the table
            insert(p); // recursively call insert() with the same argument
        } else {
            if (z != npos) {
                if (table_[z] == nullptr) {
                    table_[z] = new HashItem(p); // insert the new item
                }
            }
            ++si; // increment the size of the table
            ++in; // increment the number of insertions
        }
    }
}
// To be completed
...
{
    if (empty()) return; // if the table is empty, do nothing
    HASH_INDEX_T z = this->probe(key); // probe the table to find the index of the key
    if (z != npos && table_[z] != nullptr) { // if the key is found at a non-empty position
        table_[z]->deleted = true; // mark the item as deleted
        --si; // decrement the size of the table
    }
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
    ...
    // Increment the index of the prime number array to get the next table size
    ++mIndex_;
    in= 0; // reset the insertion count

    // Create a new temporary table
    std::vector<HashItem*> t;

    // Copy all non-deleted items from the old table to the temporary table
    for (auto& z: table_) {
        if (z == nullptr) continue; // if the item is null, skip to the next iteration
        if (z->deleted) {
            delete z; // if the item is marked as deleted, delete it from memory
        }
        else {
            t.push_back(z); // otherwise, add it to the temporary table
        }
    }

    // Create a new table with the next prime size
    table_ = std::vector<HashItem*>(CAPACITIES[mIndex_], NULL);

    // Insert all items from the temporary table into the new table
    for (auto& z: t) {
        table_[probe(z->item.first)] = z; // insert the item into the new table
        ++in; // increment the insertion count
    }
}
